"""Sensor entities for Aquarium Hub (Neptune Apex, read-only in v1).
...
from .const import DOMAIN, MANUFACTURER_APEX, PROBE_UNITS
...
async def async_setup_entry(
    ...
    coordinator: AquariumHubCoordinator = hass.data[DOMAIN][entry.entry_id]
...
class _ApexEntityBase(CoordinatorEntity[AquariumHubCoordinator], SensorEntity):
    """Shared device-grouping info so every entity lands under one device."""

    _attr_has_entity_name = True

    def __init__(self, coordinator: AquariumHubCoordinator, entry: ConfigEntry) -> None:
...
class ApexProbeSensor(_ApexEntityBase):
    ...
    def __init__(
        self, coordinator: AquariumHubCoordinator, entry: ConfigEntry, probe_name: str
    ) -> None:
...
class ApexOutputStateSensor(_ApexEntityBase):
    ...
    def __init__(
        self, coordinator: AquariumHubCoordinator, entry: ConfigEntry, output_name: str
    ) -> None:
        super().__init__(coordinator, entry)
